...
class SandwichMakerBehaviour: public Behaviour
{
    ...
    // encode the current state of the state machine
    int state = 0;
    
    // not sure why these were global, and not in the state struct.
    double T = 0;
    double t = 0;
    double aux = 0;
    int flag = 0;

    // Sharar's constant definitions

    //The constants are not defined as const becasue of the parameters of remoteApi functions.
...
public:
    ...
    float t_x = 0, t_y = 0, ts = 0;
    
    STEP_RETURN pid(float p_theta,float p_radius);
    STEP_RETURN intercept(float x, float y, float t);
    STEP_RETURN go_home();
    float epsilon_theta = 0.05, epsilon_radius = 0.01;

    // STATE MACHINE CONTROL VARIABLES
    STATES simDectectionBehaviourStm_state = TURNING_ON;
    OPERATION_STATES OPERATION_state = FINDING_FIRST_SHAPE;
    ...
    bool OPERATION_in_state = false;
    STEP_RETURN simDectectionBehaviourStm_step();
    STEP_RETURN OPERATION_step();
};
