...
void SandwichMakerBehaviour::step()
{
    //while (mstep(p, s)){}
    /* if (p->x1_event) {
        cout << "Found bread in position 1 with x coordinate: " << p->x1 << endl;
    } else {
        cout << "Not bread in position 1" << endl;
    }*/
    /* cout << "Target: 0.52, 0.10; Current: " << p->c_theta << ", " << p->c_radius << endl;
    STEP_RETURN v = pid(0.52, 0.20);
    while (v == CONTINUE) {
        v = pid(0.52, 0.20);
    }
    if (v == TERMINATED) {
        cout << "PID has terminated" << endl;
    } */
    //printf("running step()\n");
    while (simDectectionBehaviourStm_step() == CONTINUE)
    {
        //printf("%d, %d\n", simDectectionBehaviourStm_state, OPERATION_state);
    }
}

// test implementation
/* STEP_RETURN SandwichMakerBehaviour::machine_step() {
    switch(state) {
        case 0: 
        ...
        case 3: {
            ...
            STEP_RETURN v = intercept(t_x, t_y, ts);
            if (v == TERMINATED) {
                // take transition
                state++;
                return CONTINUE;
            } else {
                // stay in state and wait for next cycle
                return WAIT;
            }
        }
        case 4: {
            ...
            p->activate_gripper();
            cout << "Gripper activated" << endl;
            p->lower(p->HEIGHT_TO_SHAPE1);
            cout << "Gripper lowering" << endl;
            state++;
            return WAIT;
        }
        case 5: {
...
{
    float delta_t = p->c_time - t;
    float newx = x + 0.47;
    float newy = y + delta_t * (p->beltVelocity);
    double aux = sqrt(newx * newx + newy * newy);
    float theta = acos(newx / aux);
    float radius = aux - 0.32;
    STEP_RETURN v = pid(theta, radius);
