...
void SandwichMakerPlatform::readSensors()
{
	...
	simxGetJointPosition(clientID, rj, &c_theta, simx_opmode_buffer);
	simxGetJointPosition(clientID, pj1, &c_radius, simx_opmode_buffer);
...
void SandwichMakerPlatform::writeActuators()
{
	...
	/*
		// execute output operations and events in order

		// transmit information to v-rep. I'm not sure if this should be done in the
		// execute of each output, as it could lead to multiple accesses to the vrep
		// api and it is  not yet clear if it could have side effects beyond what is
		// expected (overriding).
        double x = px + 0.47;
        double y = py;
		...
		double r = aux - 0.32;
		double theta = acos(x/aux);
		
		simxSetJointTargetPosition(clientID,rj,theta,simx_opmode_oneshot);
		simxSetJointTargetPosition(clientID,pj1,r,simx_opmode_oneshot);
	...
	*/
	...
	// update global clock
	//c_time = c_time + step_size; //((double)simxGetLastCmdTime(clientID))/1000;
	c_time = ((double)simxGetLastCmdTime(clientID)) / 1000;

	//cout << c_time << ", " << aux << endl;
